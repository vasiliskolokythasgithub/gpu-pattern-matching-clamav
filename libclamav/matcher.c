...
#include "yara_exec.h"
#endif
#include "filetypes.h"  /* This should already be included, but let's make sure */

/* If CL_TYPE_PE etc are still not defined, add these lines */
#ifndef CL_TYPE_PE
#define CL_TYPE_PE 7  /* From filetypes.h */
#endif
#ifndef CL_TYPE_ELF
#define CL_TYPE_ELF 11
#endif
#ifndef CL_TYPE_MACHO
#define CL_TYPE_MACHO 14
#endif
                                     ...
                                     cli_ctx *ctx)
{
    ...
    cli_dbgmsg("MATCHER_RUN called: root=%p, length=%u, ctx=%p, gpu_enabled=%d\n",
           (void*)root, length, (void*)ctx, root ? root->gpu_enabled : -1);
...
cl_error_t cli_scan_fmap(cli_ctx *ctx, cli_file_t ftype, bool filetype_only, struct cli_matched_type **ftoffset, unsigned int acmode, struct cli_ac_result **acres, unsigned char *refhash)
{
    ...
    cli_dbgmsg("CLI_SCAN_FMAP called: ftype=%d, filetype_only=%d\n", ftype, filetype_only);
    ...
    cli_dbgmsg("CLI_SCAN_FMAP: Engine OK, proceeding with scan\n");
    ...
    cli_dbgmsg("CLI_SCAN_FMAP: maxpatlen=%u, about to check PE authenticode\n", maxpatlen);
    ...
    if (generic_ac_root && generic_ac_root->gpu_enabled && 
        ...
        if (full_buf) {
         ...
         int gpu_rc = gpu_scan(ctx->engine->gpu_rt, full_buf,
            ...
            else if (gpu_rc == GPU_RESULT_CLEAN) {
                ctx->gpu_ac_done = true;
                ret = CL_CLEAN;
                goto done;
            }
...
cpu_scan:
    ...
    while (offset < ctx->fmap->len) {
        ...
        if (target_ac_root) {
            ...
            bytes = MIN(ctx->fmap->len - offset, SCANBUFF);
             cli_dbgmsg("CLI_SCAN_FMAP: target_ac_root exists, calling matcher_run\n");
            if (!(buff = fmap_need_off_once(ctx->fmap, offset, bytes)))
                break;
