...
struct scan_cache {
    struct scan_cache_entry *entries;
    uint32_t count;
    uint32_t capacity;
};
#endif
...
typedef struct cli_ctx_tag {
    ...
    struct gpu_ac_hits *gpu_ac_hits;
struct gpu_hint_list {
    struct gpu_hint *hints;
    uint32_t count;
    bool valid;
};
    #ifdef HAVE_OPENCL
        bool gpu_ac_done;
        struct gpu_hint_list gpu_hints;
    #endif
   

#ifdef HAVE_OPENCL
    struct gpu_rt *gpu_rt;
    struct scan_cache *gpu_cache;  /* ← pointer member */
#endif
} cli_ctx;
