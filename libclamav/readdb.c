                                         ...
                                         int current_subsig_index, int num_subsigs, struct cli_lsig_tdb *tdb)
{
    ...
    if (hexsig[0] == '$') {
        ...
        struct cli_ac_lsig *lsig = root->ac_lsigtable[lsigid[0]];
        if (lsig) {
            lsig->has_regex = 1;
        }
...
cl_error_t cl_engine_compile(struct cl_engine *engine)
{
    ...
    #ifdef HAVE_OPENCL
    /* Initialize GPU runtime ONCE per engine */
    if (!engine->gpu_rt) {
        engine->gpu_rt = calloc(1, sizeof(struct gpu_rt));
        if (!engine->gpu_rt)
            return CL_EMEM;

       if (gpu_rt_init(engine->gpu_rt) != 0) {
            fprintf(stderr, "GPU: gpu_rt_init FAILED\n");
            free(engine->gpu_rt);
            engine->gpu_rt = NULL;
            cli_dbgmsg("GPU disabled: runtime init failed\n");
        } else {
            fprintf(stderr, "GPU: gpu_rt_init SUCCESS, gpu_rt=%p\n", (void*)engine->gpu_rt);
        }
    }
    
    /* Extract DFA for all matchers ONCE during engine compile */
    // if (engine->gpu_rt) {  // Keep this block
    //     cli_dbgmsg("GPU: Extracting DFA for all matchers (one-time setup)\n");
        
    //     for (i = 0; i < CLI_MTARGETS; i++) {
    //         if (engine->root[i]) {
    //             cli_dbgmsg("GPU: Extracting DFA for matcher %u\n", i);
    //             gpu_extract_dfa(engine->root[i]);  // Only one parameter
    //         }
    //     }
        
    //     cli_dbgmsg("GPU: DFA extraction complete, ready for scanning\n");
    // }
    #endif
...
fprintf(stderr, "ROOT type=%u: lsig_patterns=%u, simple_patterns=%u, total=%u\n",
    ...
    #if HAVE_PCRE
            if ((ret = cli_pcre_build(root,
                                    engine->pcre_match_limit,
                                    engine->pcre_recmatch_limit,
                                    engine->dconf)))
                return ret;
            TASK_COMPLETE();
    #endif
